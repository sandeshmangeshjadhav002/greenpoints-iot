...
export default function UserMenu() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <img src={currentUser.avatar} alt={currentUser.name} className="h-8 w-8 rounded-full ring-2 ring-leaf-300 dark:ring-leaf-700" />
        <span className="hidden text-sm font-medium sm:block">{currentUser.name.split(' ')[0]}</span>
      ...
      {open && (
        <div className="glass absolute right-0 z-50 mt-2 w-52 rounded-2xl p-2 shadow-glow animate-fadeUp">
          <div className="px-3 py-2">
            ...
            <p className="truncate text-xs text-leaf-700/60 dark:text-leaf-200/50">{currentUser.email}</p>
          </div>
